...
Output multiple forecasts (test)
--------------------------------
...
output.multiple <- function(isbn){  # note that this should be a 2 param fn
  ...
  num.contig <- length(na.contiguous(ts1))
  ...
  if (!is.na(isbn.prev)) {
    ...
    # Add previous edition forecast
    # Note that this doesn't use the previous
    # edition as the forecast - it needs to be aligned further with the forecast
    # period
    prev.edition <- align.previous.edition(isbn, time.series, title.data)
    output <- cbind(output, prev.edition)
