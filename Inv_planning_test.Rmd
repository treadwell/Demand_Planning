...
fcst1 = forecast(ts1, h=horizon)$mean
fcst1.df = TS2DF(fcst1)
fcst1.df


  # How Life Works
result = output.multiple(isbn, time.series.data, title.data)

print(class(result))

# 1. chack that each subforecast is generating a data frame in the appropriate format
# 2. make sure that this returns a data frame with date indices
# 3. make sure that forecasts are labeled correctly
